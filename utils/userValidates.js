  ...
  Joi.object()
    ...
    .keys({
      email: Joi.string()
        ...
        .required(),
      password: Joi.string()
        .pattern(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{4,20}$/) //* pass must include at least one upper case letter, one lower case letter, and one numeric digit. Must be at least 4 characters, no more than 8 characters
        .required(),
    })
    .validate(data);
